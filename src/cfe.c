...
// ####    Note: all of thes variables are storages in [m] or fluxes in [m/timestep]
    ...
    if(yes_simulate_discrete_soil_moisture) {  // simulating discretized soil
      for(int i = 0; i< NDISC; i++)
          soil_storage_temp_m += soil_discrete_storage_theta[i] * soil_geometry->dz_m[i];
    } else { // conceptual soil reservoir
...
//----------------------
  ...
  if (yes_simulate_discrete_soil_moisture) {  
    ...
    if(verbosity > 2 && yes_simulate_discrete_soil_moisture ) { // THIS PRODUCES A TON OF OUTPUT TO stdout
  ...
  if(0.0 < water_input_depth_m) {
    if (0.0 >= column_total_soil_moisture_deficit_m) {
      ...
      *infiltration_depth_m = (Px * (Ic / (Px + Ic)));
      ...
      else  *flux_surface_runoff_input_to_surface_routing_m=0.0;
        ...
        )
{
  ...
  *infiltration_depth_m    = 0.0;          // PDDUM
  ...
  if(tension_water_m >  max_tension_water_m) tension_water_m = max_tension_water_m; // as done in NWM - could cause volbal err.
  if(free_water_m    >  max_free_water_m)    free_water_m    = max_free_water_m;    // as done in NWM - could cause volbal err.
...
#ifdef DEBUG
    if(fabs(water_input_depth_m - (*infiltration_depth_m) - (*flux_surface_runoff_input_to_surface_routing_m)) > 1.0e-06) printf("volball err. warning: %f\n",
